...
def extract_parameters_from_history(messages: list[dict], current_state: Dict[str, Any] | None = None) -> Dict[str, Any]:
    ...
    2. ROUNDED CORNERS: If the user asks for a rounded room, curved corners, or a "round form", you MUST set `corner_radius` to a positive float (e.g. 1.0) and specify the exact corners in `rounded_corners`. CRITICAL: ONLY include the EXACT corners the user explicitly requested (e.g., ["top_right"] for top right, ["bottom_left"] for bottom left). Do NOT include extra corners that were not requested.
