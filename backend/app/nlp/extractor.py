...
def extract_parameters_from_history(messages: list[dict], current_state: Dict[str, Any] | None = None) -> Dict[str, Any]:
    ...
    2. ROUNDED CORNERS: If the user asks for a rounded room, curved corners, or a "round form", you MUST set `corner_radius` to a positive float (e.g. 1.0) and specify the exact corners in `rounded_corners` (e.g., ["top_left", "top_right", "bottom_right", "bottom_left"]).
